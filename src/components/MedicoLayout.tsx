...
const MedicoLayoutContent: React.FC<{ children: React.ReactNode; title: string }> = ({ children, title }) => {
...
const MedicoLayout: React.FC<MedicoLayoutProps> = ({ children, title = 'Médico' }) => {
    return (
        <ThemeProvider>
            <SidebarProvider userType="medico">
                <MedicoLayoutContent title={title}>
                    {children}
                </MedicoLayoutContent>
            </SidebarProvider>
        </ThemeProvider>
